...
@pytest.mark.parametrize(
    ("content", "assertion"),
    [
        ("---\ntype: concept\n---\n# Title\nBody", lambda note: note.frontmatter["type"] == "concept"),
        ("---\ntags: [alpha, beta]\n---\n# Title", lambda note: note.tags == ("alpha", "beta")),
        ("---\ntags: alpha, beta\n---\n# Title", lambda note: note.tags == ("alpha", "beta")),
        ("---\ntags: '#alpha #beta'\n---\n# Title", lambda note: note.tags == ("alpha", "beta")),
        ("---\n: broken\n---\n# Title", lambda note: note.frontmatter_parse_error is not None),
        ("no frontmatter\n# Title", lambda note: note.frontmatter == {}),
        ("---\n- not\n- mapping\n---\n# Title", lambda note: note.frontmatter == {}),
        ("# Title\nBody", lambda note: note.headings[0].text == "Title"),
        ("# Parent\nA\n## Child\nB", lambda note: note.sections[1].section_path == "Parent > Child"),
        ("```\n# Not heading\n```\n# Real", lambda note: [h.text for h in note.headings] == ["Real"]),
        ("~~~\n## Not heading\n~~~\n## Real", lambda note: [h.text for h in note.headings] == ["Real"]),
        ("# T\nSee [[Other Note]]", lambda note: note.wikilinks[0].target == "Other Note"),
        ("# T\nSee [[Other Note|alias]]", lambda note: note.wikilinks[0].alias == "alias"),
        ("# T\nBody #alpha/sub", lambda note: note.tags == ("alpha/sub",)),
        ("# T\n`#notatag`\n#yes", lambda note: note.tags == ("yes",)),
        ("# Repeat\nA\n# Repeat\nB", lambda note: [h.ordinal for h in note.headings] == [1, 2]),
        (
            "# Repeat\nA\n# Repeat\nB",
        ...
        (
            "# Title\r\nBody\r\n",
            lambda note: note.normalized_content_hash
            == _parse("# Title\nBody\n").normalized_content_hash,
        ),
        (
            "# Title\r\nBody\r\n",
            lambda note: note.raw_content_hash != _parse("# Title\nBody\n").raw_content_hash,
        ),
        ("# Title\n\n   \n", lambda note: note.blocks == ()),
        ("Body without heading", lambda note: note.sections[0].section_key == "wiki/note.md#root#1"),
        ("# Title ###\nBody", lambda note: note.headings[0].text == "Title"),
        ("# Hello, World!\nA\n# Hello World\nB", lambda note: [h.ordinal for h in note.headings] == [1, 2]),
        (
            "# T\n| A | B |\n| - | - |\n| 1 | 2 |",
            lambda note: len(note.blocks) == 1 and "| 1 | 2 |" in note.blocks[0].content,
        ),
        (
            "# T\n```python\nprint('x')\n```",
            lambda note: len(note.blocks) == 1 and "```python" in note.blocks[0].content,
        ),
        (
        ...
        (
            "# T\n" + ("word " * 5000),
            lambda note: all(block.token_count_estimate <= 1000 for block in note.blocks),
        ),
        (
            "---\ntags: [front]\n---\n# T\nBody #inline",
...
def test_block_segmentation_is_deterministic_and_non_overlapping() -> None:
    content = "# T\n" + "\n\n".join(f"paragraph {index} " + ("word " * 90) for index in range(12))

    first = _parse(content)
    second = _parse(content)

    assert [block.block_key for block in first.blocks] == [block.block_key for block in second.blocks]
...
def test_raw_and_normalized_hashes_accept_bytes_input() -> None:
    ...
    assert parsed.normalized_content_hash == _parse("# Bytes\nBody\n").normalized_content_hash
