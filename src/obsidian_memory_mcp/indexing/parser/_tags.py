...
from obsidian_memory_mcp.markdown import is_fence_line, next_fence_state, tag_values_from_field
...
def extract_tags(frontmatter: dict[str, Any], body_lines: list[str]) -> tuple[str, ...]:
    tags: set[str] = set()
    tags.update(_normalize_index_tags(tag_values_from_field(frontmatter.get("tags"))))
...
def _normalize_index_tags(values: set[str]) -> set[str]:
    return {_normalize_tag(tag) for tag in values}
